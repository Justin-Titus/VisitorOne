...
export default function RegisterVisitor() {
  ...
  useEffect(() => {
    api.get('/employees').then((res) => {
      setEmployees(res.data.data.filter((e: any) => e.status === 'active'));
    }).catch(() => {});
  }, []);
  ...
  return (
    <AnimatedPage className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Form Column */}
        <form onSubmit={(e) => void handleSubmit(e)} className="lg:col-span-7 space-y-5">
          ...
          {showMobileBadge && (
            <div className="lg:hidden mt-4 p-4 rounded-2xl bg-slate-100/70 dark:bg-black/30 border border-slate-200 dark:border-white/[0.08] space-y-3">
              ...
              <VisitorBadgeCard req={previewReq as any} userRole="preview" isInteractive={true} />
