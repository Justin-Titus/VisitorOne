...
export default function LoginPage() {
  ...
  const handleForgotPassword = () => {
    toast('For password recovery or clearance reset, please contact your system administrator.', {
      icon: '🔒',
      duration: 4000,
    });
  };
        ...
        <div className="relative rounded-3xl bg-white/90 dark:bg-[#0c101b]/90 border border-slate-200/80 dark:border-white/[0.08] shadow-2xl backdrop-blur-xl overflow-hidden">
          ...
          <div className="px-8 pb-7 space-y-5">
            <form onSubmit={handleSubmit} className="space-y-4">
              {/* Email Field */}
              <div>
                <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-1.5">
                  Work Email
                </label>
              ...
              <button
                type="submit"
                disabled={loading}
                className="relative w-full h-11 rounded-xl bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white text-xs font-bold tracking-wide transition-all duration-150 shadow-md shadow-indigo-600/20 hover:shadow-lg hover:shadow-indigo-600/30 flex items-center justify-center gap-2 group disabled:opacity-70 disabled:cursor-not-allowed mt-2"
              >
                {loading ? (
            ...
            {/* Quick Demo Access Clearance Profiles */}
                ...
                {demoProfiles.map((p) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative z-10 py-2 px-1.5 rounded-xl font-semibold text-xs transition-colors duration-150 text-center select-none ${
                        isSelected
                    ...
                    >
                      {isSelected && (
                        <motion.div
                          layoutId="loginDemoActivePill"
                          className="absolute inset-0 bg-white dark:bg-[#151c2e] rounded-xl shadow-xs border border-indigo-500/30"
                          transition={{ type: 'spring', stiffness: 500, damping: 35 }}
                        />
                      )}
                      <span className="relative z-10 block">
                        <span
                          className={`inline-block text-[9px] font-mono font-bold px-1.5 py-0.2 rounded border mb-0.5 ${p.badgeClass}`}
                        >
                          {p.label}
                        </span>
          ...
          {/* Card Footer: Security Trust Marks */}
