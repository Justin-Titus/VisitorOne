...
export default function VisitorRequests() {
  ...
  const { data: queryData, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const params = { page, limit: 12, ...debouncedTextFilters, ...immediateFilters };
                    ...
                    {requests.map((req) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td onClick={() => openDetail(req)} className="px-4 py-3 font-mono font-bold text-slate-500 dark:text-slate-400">
                            #{req._id.slice(-6).toUpperCase()}
                          </td>
                          <td onClick={() => openDetail(req)} className="px-4 py-3">
                          ...
                          <td onClick={() => openDetail(req)} className="px-4 py-3 text-slate-600 dark:text-slate-400 max-w-[160px] truncate">{req.purpose}</td>
                          <td onClick={() => openDetail(req)} className="px-4 py-3 text-slate-600 dark:text-slate-400">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1">
                              ...
                              {user.role === 'employee' && req.status === 'pending' && (
                                <>
                                  <button onClick={() => handleAction('approve', req._id)} className="w-7 h-7 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex items-center justify-center transition-colors" title="Approve"><CheckCircle size={13} /></button>
                                  <button onClick={() => openDetail(req)} className="w-7 h-7 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 flex items-center justify-center transition-colors" title="Reject"><XCircle size={13} /></button>
                                </>
                              )}
                              {user.role === 'receptionist' && req.status === 'approved' && (
                                <button onClick={() => handleAction('check-in', req._id)} className="w-7 h-7 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-600 dark:text-cyan-400 flex items-center justify-center transition-colors" title="Check In"><LogIn size={13} /></button>
                              )}
                              {user.role === 'receptionist' && req.status === 'checked_in' && (
                                <button onClick={() => handleAction('check-out', req._id)} className="w-7 h-7 rounded-lg bg-slate-500/10 hover:bg-slate-500/20 text-slate-600 dark:text-slate-400 flex items-center justify-center transition-colors" title="Check Out"><LogOut size={13} /></button>
                              )}
                            </div>
                          </td>
                        </tr>
