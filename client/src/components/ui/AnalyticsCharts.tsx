...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    let displayName: string = typeof label === 'string' ? label : (payload[0]?.name as string) ?? 'Count';

    if (typeof displayName === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(displayName)) {
      const [y, m, d] = displayName.split('-');
      const dateObj = new Date(Number(y), Number(m) - 1, Number(d));
      displayName = dateObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    }

    const color = (payload[0]?.color as string) ?? ((payload[0]?.payload as { fill?: string })?.fill) ?? '#818cf8';
    ...
    return (
      <div className="bg-slate-900/90 backdrop-blur-md text-white text-xs p-2.5 rounded-xl border border-white/10 shadow-xl">
        ...
        <p className="text-slate-200 mt-1">
          <span className="font-semibold" style={{ color }}>{payload[0]?.value as number}</span> visitors
        </p>
      </div>
    );
  }
...
export function DailyTrendsChart({ data = [] }: { data?: DateCount[] }) {
  ...
  if (sortedData.length > 0) {
    ...
    if (firstItem && lastItem) {
      const [sy, sm, sd] = firstItem.date.split('-').map(Number);
      const [ey, em, ed] = lastItem.date.split('-').map(Number);
      let currDate = new Date(sy, (sm ?? 1) - 1, sd);
      ...
      while (currDate <= endDate) {
        ...
        filledData.push({ date: dateString, count: existing ? existing.count : 0 });
...
export function PeakHoursChart({ data = [] }: { data?: HourCount[] }) {
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, { once: true, margin: '-50px' });
...
export function StatusDistributionChart({ summary = {} }: { summary?: StatusSummary }) {
  const chartData = [
...
export function DepartmentChart({ data = [] }: { data?: DepartmentCount[] }) {
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, { once: true, margin: '-50px' });
