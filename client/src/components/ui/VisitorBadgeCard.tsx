...
export interface VisitRequestData {
  ...
  purpose?: string;
  visitor?: { name?: string; phone?: string; company?: string };
  employeeToVisit?: { name?: string; department?: string };
}

interface VisitorBadgeCardProps {
  req: VisitRequestData;
  userRole: string;
  onOpenDetail?: (req: VisitRequestData) => void;
  onAction?: (action: string, id: string) => void;
  actionLoading?: string;
  isInteractive?: boolean;
  onPrint?: (req: VisitRequestData) => void;
}
      ...
      >
        ...
        {userRole === 'preview' ? (
          <div className="p-2.5 px-4 bg-slate-50/80 dark:bg-white/[0.02] border-t border-slate-200/80 dark:border-white/[0.06] flex items-center justify-between text-[11px] font-mono text-slate-500 relative z-10">
            ...
            <span className="text-[10px] text-slate-400 dark:text-slate-500">UNISSUED PASS</span>
          </div>
        ) : (
          isInteractive && (
